...
# External IP lookup service URL (e.g., ipinfo.io)
IP_LOOKUP_SERVICE_URL =  "https://ipinfo.io"

def reverse_ip(ip):
    """Reverse the segments of an IP address."""
    return '.'.join(reversed(ip.split('.')))

@app.route('/reverse-public-ip', methods=['POST'])
...
#     """
    ...
    try:
        ...
        # Reverse the IP address in the response
        public_ip = ip_info.get('ip', ip_address)
        reversed_ip = reverse_ip(public_ip)

        ip_info['reversed_ip'] = reversed_ip
